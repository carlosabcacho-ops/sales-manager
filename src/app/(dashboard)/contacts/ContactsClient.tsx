...
import { Search, Phone, TrendingUp, Calendar, Flame, Wind, Snowflake } from 'lucide-react'
...
const labelConfig: Record<ContactLabel, {
...
export function ContactsClient({ contacts }: ContactsClientProps) {
  ...
  const hotCount = contacts.filter(c => c.label === 'HOT').length

  return (
    <div className="p-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[#e6edf3]">Contatos</h1>
        <p className="text-sm text-[#8b949e] mt-1">
          {contacts.length} total · {hotCount} leads quentes
        </p>
      </div>
      ...
      <div className="flex items-center gap-3 mb-5">
        <div className="flex-1 relative max-w-sm">
          ...
          <input
            ...
            placeholder="Buscar contatos ou endereços..."
      ...
      <div className="rounded-xl border border-[#30363d] bg-[#161b22] overflow-hidden">
        {filtered.length === 0 ? (
          <div className="py-16 text-center">
            <p className="text-sm text-[#8b949e]">Nenhum contato encontrado.</p>
            {contacts.length === 0 && (
              <p className="text-xs text-[#484f58] mt-1">Os contatos aparecerão aqui quando forem importados via GHL.</p>
            )}
          </div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#30363d]">
                {['Contato', 'Localização', 'Status', 'Calls', 'Score Médio', 'Último Contato'].map(h => (
              ...
              {filtered.map((c, i) => {
                ...
                return (
                  <tr
                    key={c.id}
                    className={cn(
                      'border-b border-[#21262d] hover:bg-[#1c2333] transition-colors cursor-pointer',
                    ...
                    <td className="px-4 py-3.5">
                      {c.avg_score > 0 ? (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className={cn('text-sm font-bold tabular-nums', scoreColor(c.avg_score))}>
